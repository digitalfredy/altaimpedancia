\message{ !name(alternativa.tex)}\documentclass{beamer}
\mode<presentation>
{
...
\hypersetup{pdfkeywords=open free design source hardware colombia Software Libre abierto 2009 AltaImpedancia alta impedancia electrónica alternativa}
\title[Alternativas para electrónica]{Software Libre una alternativa para soluciones en el campo de la electrónica}
\author[]{Jorge~Ernesto~Guevara~Cuenca}
...
\date[Software Libre en la educación]
{Primer congreso departamental de software libre en la educación\\
  Duitama - 12, 13 y 14 de agosto de 2009}
\subject{Software Libre alternativa para electrónica}
...
\message{ !name(alternativa.tex) !offset(-3) }
...
% \begin{frame}{Simulación digital}{\alert{vhdl2vl} - \url{http://doolittle.icarus.com/~larry/vhd2vl}}
%   \begin{itemize}
%   \item Traductor de código sintetizable en VHDL a Verilog HDL.
%     \pause
%   \item \alert{wcalc} Herramienta para análisis y síntesis de estructuras de líneas de transmision y componentes relacionados.
%   \end{itemize}
% \end{frame}
...
%   \end{figure}
%   \alert{kicad} Permite crear y nombrar proyectos e iniciar la mayor parte de los componentes de KiCad.
% \end{frame}
...
%   \end{figure}
%   \alert{eeschema} Permite crear y nombrar proyectos e iniciar la mayor parte de los componentes de KiCad.
% \end{frame}

% \begin{frame}{kicad}{El selector de componentes usados en el diseño del circuito}
%   \alert{cvpcb} Es una herramienta que permite asignar huellas/formas (footprint) de circuito impreso a los símbolos esquemáticos del diseño en eeschema.
% \end{frame}
...
%   \end{figure}
%   \alert{pcbnew} Contiene el editor de módulos (representación gráfica de la forma/huella), donde se pueden cambiar y crear los módulos.
% \end{frame}
...
%   \end{figure}
%   \alert{gerbview} Visor Gerber para verificar los archivos gerber (son los archivos .pho en KiCad), que es el formato de archivos que generalmente solicitan los fabricantes de circuitos impresos.
% \end{frame}
...
% \begin{frame}{Open Circuit Design}{Herramientas}
%   \begin{itemize}
%   \item \alert{netgen} Es una herramienta para comparar netlists, un proceso conocido como LVS, que significa "Diseño vs Esquema".
...
Hay suficiente software EDA libre disponible entonces la tarea es usarlo en las aulas de clase 
...
\message{ !name(alternativa.tex) !offset(-862) }
